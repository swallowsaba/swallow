...
'use strict';
// === YAML Parser ===
function parseYAML(t){const r={zones:[],nodes:[],connections:[],environments:[]};let s=null,it=null;for(const l of t.split('\n')){const tr=l.trim();if(!tr||tr[0]==='#')continue;const ind=l.search(/\S/);if(ind===0&&tr.endsWith(':')){const k=tr.slice(0,-1).trim();if(k in r)s=k;it=null;continue}if(!s)continue;if(tr.startsWith('- ')){it={};r[s].push(it);const kv=tr.slice(2).trim();if(kv&&kv.includes(':')){const ci=kv.indexOf(':'),k=kv.slice(0,ci).trim(),v=kv.slice(ci+1).trim();if(k)it[k]=pv(v)}continue}if(it&&tr.includes(':')){const ci=tr.indexOf(':'),k=tr.slice(0,ci).trim(),v=tr.slice(ci+1).trim();if(k)it[k]=pv(v)}}return r}
function pv(s){if(s===''||s==null)return'';if((s[0]==='"'&&s.endsWith('"'))||(s[0]==="'"&&s.endsWith("'")))return s.slice(1,-1);if(s[0]==='['&&s.endsWith(']')){const i=s.slice(1,-1).trim();return i?i.split(',').map(x=>pv(x.trim())):[]}if(s[0]==='{'&&s.endsWith('}')){const i=s.slice(1,-1).trim();if(!i)return{};const o={};for(const pair of i.split(',')){const ci=pair.indexOf(':');if(ci<0)continue;const k=pair.slice(0,ci).trim(),v=pair.slice(ci+1).trim();if(k)o[k]=pv(v)}return o}if(s==='true')return!0;if(s==='false')return!1;if(s==='null')return null;if(/^-?\d+(\.\d+)?$/.test(s))return+s;return s}
function serYAML(d){let y='';for(const s of['environments','zones','nodes','connections']){const items=d[s];if(!items||!items.length)continue;y+=s+':\n';for(const item of items){let f=!0;for(const k of Object.keys(item)){y+=(f?'  - ':'    ')+k+': '+sv(item[k])+'\n';f=!1}}y+='\n'}return y.trimEnd()+'\n'}
function sv(v){if(v==null)return'';if(typeof v==='boolean')return v?'true':'false';if(typeof v==='number')return''+v;if(Array.isArray(v))return'['+v.map(sv).join(', ')+']';if(typeof v==='object'){const parts=[];for(const[k,vv]of Object.entries(v))parts.push(k+': '+sv(vv));return'{'+parts.join(', ')+'}'}const s=''+v;if(s.includes('#')||s.includes(':')||s.includes('"')||s.includes("'")||s.includes(',')||s.includes('[')||s.includes(']')||s.includes('{')||s.includes('}')||s.startsWith(' ')||s.endsWith(' ')||s==='true'||s==='false'||s==='null'||/^-?\d+(\.\d+)?$/.test(s))return'"'+s.replace(/\\/g,'\\\\').replace(/"/g,'\\"')+'"';return s}
...
// === State ===
let S={zones:[],nodes:[],connections:[],environments:[]},sel=null,cm=null;
// Set of environment IDs currently checked for visibility. null = show all
let activeEnvs=null;
// Port display layout: 'vertical' (default) or 'horizontal'
let portLayout='vertical';

// === Environment Management ===
function envColorFor(id){const e=S.environments.find(e=>e.id===id);return e?(e.color||'#58a6ff'):'#888'}
// Check if entity is visible based on activeEnvs
function entityVisible(ent){
  // If no envs defined at all, show everything
  if(!S.environments||!S.environments.length)return true;
  // If no filter active, show everything
  if(!activeEnvs||!activeEnvs.size)return true;
  const envs=ent.environments||[];
  // If entity has no env assignment, always show
  if(!envs.length)return true;
  // Show if any of its envs is active
  return envs.some(e=>activeEnvs.has(e));
}
// Get port for current filter: if single env active, show env-specific port; else show all
function getConnPortDisplay(c){
  const portMap=c.ports||{};
  const entries=[];
  const envSet=activeEnvs&&activeEnvs.size?activeEnvs:null;
  const envKeys=Object.keys(portMap);
  if(envKeys.length){
    const show=envSet?envKeys.filter(k=>envSet.has(k)):envKeys;
    for(const envId of show){
      if(portMap[envId]!=null){
        const env=S.environments.find(e=>e.id===envId);
        entries.push({env:env?(env.label||env.id):envId,port:String(portMap[envId])});
      }
    }
    // All same port → collapse to single portOnly
    if(entries.length>1){
      const vals=new Set(entries.map(e=>e.port));
      if(vals.size===1)return[{port:entries[0].port}];
    }
    // Single env entry → strip env name (no need to display)
    if(entries.length===1&&entries[0].env)return[{port:entries[0].port}];
  }else if(c.port!=null&&String(c.port)){
    entries.push({port:String(c.port)});
  }
  return entries;
}
...
function renderEnvBar(){
  const bar=$('envbar');
  if(!S.environments||!S.environments.length){bar.innerHTML='';return}
  let h='<span class="env-label">環境フィルタ:</span>';
  h+='<button class="env-all" id="env-all-btn">すべて表示</button>';
  for(const e of S.environments){
    const active=activeEnvs&&activeEnvs.has(e.id);
    h+=`<label class="env-chip${active?' active':''}" data-env="${esc(e.id)}"><input type="checkbox" ${active?'checked':''}><span class="env-chip-dot" style="background:${e.color||'#58a6ff'}"></span>${esc(e.label||e.id)}</label>`;
  }
  // Layout toggle
  h+='<span style="margin-left:auto"></span>';
  h+=`<span class="env-label">ポート表示:</span>`;
  h+=`<button class="env-all${portLayout==='vertical'?' active-layout':''}" id="env-layout-v" title="縦並び">縦</button>`;
  h+=`<button class="env-all${portLayout==='horizontal'?' active-layout':''}" id="env-layout-h" title="横並び">横</button>`;
  bar.innerHTML=h;
  bar.querySelectorAll('.env-chip').forEach(chip=>{
    const eid=chip.dataset.env;
    chip.addEventListener('click',ev=>{
      ev.preventDefault();
      if(!activeEnvs)activeEnvs=new Set();
      if(activeEnvs.has(eid))activeEnvs.delete(eid);else activeEnvs.add(eid);
      if(activeEnvs.size===0)activeEnvs=null;
      renderEnvBar();render();
    });
  });
  $('env-all-btn').addEventListener('click',()=>{activeEnvs=null;renderEnvBar();render()});
  $('env-layout-v').addEventListener('click',()=>{portLayout='vertical';renderEnvBar();render()});
  $('env-layout-h').addEventListener('click',()=>{portLayout='horizontal';renderEnvBar();render()});
}

function openEnvDlg(){renderEnvList();$('env-dlg').classList.add('open')}
function closeEnvDlg(){$('env-dlg').classList.remove('open')}

function renderEnvList(){
  const list=$('env-list');
  if(!S.environments.length){list.innerHTML='<div style="color:var(--text-muted);text-align:center;padding:20px;font-size:12px">環境が登録されていません</div>';return}
  list.innerHTML=S.environments.map((e,i)=>`<div class="env-row" data-idx="${i}"><input type="color" class="env-col" value="${e.color||'#58a6ff'}"><input type="text" class="env-id" placeholder="ID" value="${esc(e.id)}"><input type="text" class="env-label-in" placeholder="Label" value="${esc(e.label||'')}"><button class="env-row-del">&times;</button></div>`).join('');
  list.querySelectorAll('.env-row').forEach(row=>{
    const idx=+row.dataset.idx;
    row.querySelector('.env-col').addEventListener('input',e=>{pushUndo();S.environments[idx].color=e.target.value;sync();renderEnvBar();render()});
    row.querySelector('.env-id').addEventListener('input',e=>{
      const old=S.environments[idx].id;S.environments[idx].id=e.target.value;
      // Update refs in zones/nodes/connections
      S.zones.forEach(z=>{if(z.environments)z.environments=z.environments.map(x=>x===old?e.target.value:x)});
      S.nodes.forEach(n=>{if(n.environments)n.environments=n.environments.map(x=>x===old?e.target.value:x)});
      S.connections.forEach(c=>{if(c.ports&&c.ports[old]!=null){c.ports[e.target.value]=c.ports[old];delete c.ports[old]}});
      if(activeEnvs&&activeEnvs.has(old)){activeEnvs.delete(old);activeEnvs.add(e.target.value)}
      sync();renderEnvBar();render();
    });
    row.querySelector('.env-label-in').addEventListener('input',e=>{S.environments[idx].label=e.target.value;sync();renderEnvBar();render()});
    row.querySelector('.env-row-del').addEventListener('click',()=>{
      pushUndo();const removedId=S.environments[idx].id;
      S.environments.splice(idx,1);
      // Clean up references
      S.zones.forEach(z=>{if(z.environments)z.environments=z.environments.filter(x=>x!==removedId)});
      S.nodes.forEach(n=>{if(n.environments)n.environments=n.environments.filter(x=>x!==removedId)});
      S.connections.forEach(c=>{if(c.ports)delete c.ports[removedId]});
      if(activeEnvs)activeEnvs.delete(removedId);
      sync();renderEnvList();renderEnvBar();render();
    });
  });
}
...
function addEnv(){
  ...
  const defaultColors=['#58a6ff','#3fb950','#d29922','#f85149','#a371f7','#ff7b72','#79c0ff','#56d364'];
  S.environments.push({id:'env-'+n,label:'環境'+n,color:defaultColors[(n-1)%defaultColors.length]});
  sync();renderEnvList();renderEnvBar();render();
}
...
function rConns(){
  ...
  // Helper: is endpoint (node or zone) visible
  function epVisible(id){
    const n=S.nodes.find(n=>n.id===id);if(n)return entityVisible(n);
    const z=S.zones.find(z=>z.id===id);if(z)return entityVisible(z);
    return false;
  }
  const pairMap={};
  for(let i=0;i<S.connections.length;i++){const c=S.connections[i];
    if(!entityVisible(c))continue;
    const fs=Array.isArray(c.from)?c.from:[c.from],ts=Array.isArray(c.to)?c.to:[c.to];
    for(const f of fs)for(const t of ts){
      if(!epVisible(f)||!epVisible(t))continue;
      const pk=[f,t].sort().join('::');if(!pairMap[pk])pairMap[pk]=[];pairMap[pk].push({fi:f,ti:t,c,idx:i})
    }
  }
  ...
  for(const lb of labels){
    const tl=lb.tw,th=lb.th;
    lC.appendChild(mk('rect',{class:'conn-bg',x:lb.x-tl/2,y:lb.y-th/2,width:tl,height:th}));
    const lineH=13;
    const startY=lb.y-(lb.lines.length-1)*lineH/2;
    for(let i=0;i<lb.lines.length;i++){
      const t=mk('text',{class:'conn-lbl',x:lb.x,y:startY+i*lineH,'text-anchor':'middle','dominant-baseline':'central',fill:lb.col});
      t.textContent=lb.lines[i];
      lC.appendChild(t);
    }
  }
...
function dConnLine(fid,tid,c,idx,offset){
  ...
  lC.appendChild(cg);
  const entries=getConnPortDisplay(c);
  const proto=c.protocol||'';
  const layout=c.portLayout||portLayout;
  let lines=[];
  if(entries.length){
    const hasEnv=entries.some(e=>e.env);
    if(!hasEnv){
      // No env labels → single line: PROTO:PORT
      const port=entries[0].port;
      lines.push(proto?(proto+':'+port):port);
    }else{
      // Env-specific entries
      const envSegs=entries.map(e=>e.env?(e.env+':'+e.port):e.port);
      if(layout==='vertical'){
        if(proto)lines.push(proto);
        for(const s of envSegs)lines.push(s);
      }else{
        lines.push((proto?proto+' ':'')+envSegs.join(' / '));
      }
    }
  }else if(proto){
    lines.push(proto);
  }
  if(!lines.length)return{cp,eps:[epFrom,epTo],label:null};
  const maxLen=Math.max(...lines.map(s=>s.length));
  const tw=maxLen*6.5+10,th=lines.length*13+5;
  return{cp,eps:[epFrom,epTo],label:{x:lx,y:ly,lines,col,tw,th}};
}
...
// Generate env checkbox list HTML for given entity
function envCheckListHtml(entityEnvs){
  if(!S.environments.length)return '<div style="color:var(--text-muted);font-size:11px;padding:4px">環境未登録</div>';
  const sel=new Set(entityEnvs||[]);
  return '<div class="env-check-list">'+S.environments.map(e=>`<label class="env-check-row"><input type="checkbox" data-env="${esc(e.id)}" ${sel.has(e.id)?'checked':''}><span class="env-check-row-dot" style="background:${e.color||'#58a6ff'}"></span>${esc(e.label||e.id)}</label>`).join('')+'</div>';
}
// Attach env checkbox listeners
function bindEnvChecks(ppb,getEnt){
  ppb.querySelectorAll('.env-check-row input').forEach(cb=>{
    cb.addEventListener('change',()=>{
      const ent=getEnt();if(!ent)return;
      const eid=cb.dataset.env;
      if(!ent.environments)ent.environments=[];
      if(cb.checked){if(!ent.environments.includes(eid))ent.environments.push(eid)}
      else{ent.environments=ent.environments.filter(x=>x!==eid)}
      if(!ent.environments.length)delete ent.environments;
      sync();render();
    });
  });
}
...
<div class="ppf"><label class="ppl">環境 (複数選択可)</label>${envCheckListHtml(z.environments)}</div>`;
$('ppa').innerHTML='<button class="ppdel" id="ppd">削除</button>';bZ(id);bindEnvChecks($('ppb'),()=>S.zones.find(zz=>zz.id===id));oPP()}
...
<div class="ppf"><label class="ppl">環境 (複数選択可)</label>${envCheckListHtml(n.environments)}</div>`;
$('ppa').innerHTML='<button class="ppdel" id="ppd">削除</button>';bN(id);bindEnvChecks($('ppb'),()=>S.nodes.find(nn=>nn.id===id));oPP()}
...
function spC(idx){const c=S.connections[idx];if(!c)return;$('ppt').textContent='Connection 設定';
  ...
  // Per-environment port inputs
  let portsHtml='';
  if(S.environments.length){
    portsHtml='<div class="ppf"><label class="ppl">環境別ポート番号</label><div class="port-env-list">';
    for(const e of S.environments){
      const pval=(c.ports&&c.ports[e.id]!=null)?c.ports[e.id]:'';
      portsHtml+=`<div class="port-env-row"><label><span class="env-check-row-dot" style="display:inline-block;background:${e.color||'#58a6ff'};margin-right:4px"></span>${esc(e.label||e.id)}</label><input type="text" data-env-port="${esc(e.id)}" value="${esc(String(pval))}" placeholder="0-65536"></div>`;
    }
    portsHtml+='</div></div>';
  }
  // Per-connection port layout
  const layoutVal=c.portLayout||'';
  const layoutHtml=S.environments.length?`<div class="ppf"><label class="ppl">ポート表示 (この矢印)</label><select class="pps" data-p="portLayout"><option value=""${layoutVal===''?' selected':''}>全体設定に従う</option><option value="vertical"${layoutVal==='vertical'?' selected':''}>縦</option><option value="horizontal"${layoutVal==='horizontal'?' selected':''}>横</option></select></div>`:'';
  $('ppb').innerHTML=`
<div id="ppb-err" style="display:none"></div>
...
<div class="ppr"><div class="ppf"><label class="ppl">Protocol</label><input class="ppi" data-p="protocol" value="${esc(c.protocol||'')}"></div><div class="ppf"><label class="ppl">Port (既定)</label><input class="ppi" type="text" data-p="port" value="${esc(c.port!=null?String(c.port):'')}" placeholder="0-65536"></div></div>
${portsHtml}
${layoutHtml}
...
<div class="ppf"><label class="ppl">環境 (複数選択可)</label>${envCheckListHtml(c.environments)}</div>`;
  ...
  $('ppa').innerHTML='<button class="ppdel" id="ppd">削除</button>';bC(idx);bindEnvChecks($('ppb'),()=>S.connections[idx]);
  // Auto-sync: ports with values → auto-check their envs
  function syncPortsToEnvs(){
    const cc=S.connections[idx];if(!cc||!cc.ports)return;
    let changed=false;
    for(const eid of Object.keys(cc.ports)){
      if(cc.ports[eid]!=null&&cc.ports[eid]!==0){
        if(!cc.environments)cc.environments=[];
        if(!cc.environments.includes(eid)){cc.environments.push(eid);changed=true}
      }
    }
    if(changed){refreshEnvChecks();sync();render()}
  }
  syncPortsToEnvs();
  // Refresh env checkbox UI for this connection
  function refreshEnvChecks(){
    const cc=S.connections[idx];if(!cc)return;
    const sel=new Set(cc.environments||[]);
    $('ppb').querySelectorAll('.env-check-row input[data-env]').forEach(cb=>{cb.checked=sel.has(cb.dataset.env)});
  }
  // Refresh all port inputs for this connection
  function refreshPortInputs(){
    const cc=S.connections[idx];if(!cc)return;
    $('ppb').querySelectorAll('[data-env-port]').forEach(inp=>{
      const eid=inp.dataset.envPort;
      inp.value=(cc.ports&&cc.ports[eid]!=null)?cc.ports[eid]:'';
    });
  }
  // Validation: check that checked envs have ports, and at least one env is selected (when envs exist)
  function validateConn(){
    const cc=S.connections[idx];if(!cc)return;
    const errs=[];
    const envs=cc.environments||[];
    // Rule 1: must belong to at least one env (only when envs are defined)
    if(S.environments.length&&!envs.length)errs.push('環境を最低1つ選択してください');
    // Rule 2: checked env without env-specific port AND no default port → error
    const missingEnvPorts=[];
    for(const eid of envs){
      const hasEnvPort=cc.ports&&cc.ports[eid]!=null&&String(cc.ports[eid]);
      const hasDefPort=cc.port!=null&&String(cc.port);
      if(!hasEnvPort&&!hasDefPort){
        const env=S.environments.find(e=>e.id===eid);
        missingEnvPorts.push(env?(env.label||env.id):eid);
      }
    }
    if(missingEnvPorts.length)errs.push('環境ポート未設定: '+missingEnvPorts.join(', '));
    // Display
    const box=$('ppb-err');
    if(box){
      if(errs.length){box.innerHTML=errs.map(e=>`⚠ ${esc(e)}`).join('<br>');box.style.display='block'}
      else box.style.display='none';
    }
    // Highlight missing env port inputs
    $('ppb').querySelectorAll('[data-env-port]').forEach(inp=>{
      const eid=inp.dataset.envPort;
      const isChecked=envs.includes(eid);
      const hasPort=cc.ports&&cc.ports[eid]!=null&&String(cc.ports[eid]);
      const hasDefPort=cc.port!=null&&String(cc.port);
      inp.classList.toggle('err',isChecked&&!hasPort&&!hasDefPort);
    });
  }
  // Port validation: empty, or comma-separated numbers 0-65536
  function validPort(s){
    if(!s||s==='')return true;
    return s.split(',').every(p=>{const n=+p.trim();return Number.isInteger(n)&&n>=0&&n<=65536});
  }
  // Bind env-specific port inputs: entering env port → clear Port(既定)
  $('ppb').querySelectorAll('[data-env-port]').forEach(inp=>{
    inp.addEventListener('input',()=>{
      const cc=S.connections[idx];if(!cc)return;
      const eid=inp.dataset.envPort,v=inp.value.trim();
      inp.classList.toggle('err',!!v&&!validPort(v));
      if(!cc.ports)cc.ports={};
      if(v===''){
        delete cc.ports[eid];
      }else{
        cc.ports[eid]=v;
        // SYNC: auto-check this env
        if(!cc.environments)cc.environments=[];
        if(!cc.environments.includes(eid)){cc.environments.push(eid);refreshEnvChecks()}
        // SYNC: clear Port(既定) — env-specific overrides default
        if(cc.port!=null){delete cc.port;refreshDefPort()}
      }
      if(Object.keys(cc.ports).length===0)delete cc.ports;
      sync();render();validateConn();
    });
  });
  // SYNC: env checkbox
  $('ppb').querySelectorAll('.env-check-row input[data-env]').forEach(cb=>{
    cb.addEventListener('change',()=>{
      const cc=S.connections[idx];if(!cc)return;
      const eid=cb.dataset.env;
      if(!cb.checked&&cc.ports&&cc.ports[eid]!=null){
        delete cc.ports[eid];
        if(Object.keys(cc.ports).length===0)delete cc.ports;
        refreshPortInputs();
      }
  ...
  // Refresh Port(既定) input from data
  function refreshDefPort(){
    const di=$('ppb').querySelector('[data-p="port"]');
    if(di){const cc=S.connections[idx];di.value=cc&&cc.port!=null?String(cc.port):''}
  }
  // Port(既定) change → clear all env ports, refresh
  const defPortInp=$('ppb').querySelector('[data-p="port"]');
  if(defPortInp){
    defPortInp.addEventListener('input',()=>{
      defPortInp.classList.toggle('err',!!defPortInp.value.trim()&&!validPort(defPortInp.value.trim()));
      // bC already deleted cc.ports; refresh UI to clear env port fields
      refreshPortInputs();
      validateConn();
    });
  }
...
function bC(idx){const ppb=$('ppb');ppb.querySelectorAll('.ppi,.pps,.ppchki,.ppci,input[type=range]').forEach(inp=>{const ev=inp.type==='checkbox'?'change':'input';inp.addEventListener(ev,()=>{const c=S.connections[idx];if(!c)return;const p=inp.dataset.p;
  if(p==='bidirectional')c.bidirectional=inp.checked;
    ...
    if(v){c.port=v;delete c.ports}else{delete c.port}
  }
  else if(p==='bend'){c.bend=+inp.value||0;if(c.bend===0)delete c.bend}
  else if(p==='portLayout'){if(inp.value)c.portLayout=inp.value;else delete c.portLayout}
...
function snapshot(){return JSON.stringify({zones:S.zones,nodes:S.nodes,connections:S.connections,environments:S.environments})}
function restoreSnap(snap){const d=JSON.parse(snap);S.zones=d.zones;S.nodes=d.nodes;S.connections=d.connections;S.environments=d.environments||[]}
...
function applyY(){try{const p=parseYAML(yamlEd.value);S.zones=p.zones||[];S.nodes=p.nodes||[];S.connections=p.connections||[];S.environments=p.environments||[];clrSel();renderEnvBar();render();toast('反映しました')}catch(e){toast('YAML解析エラー: '+e.message,1)}}
...
connections:
  ...
  - from: web01
    to: app-server
    protocol: HTTP
    ports: {prod: 8080, stg: 18080, dev: 3000}
    bidirectional: false
  - from: web02
    to: app-server
    protocol: HTTP
    ports: {prod: 8080, stg: 18080}
    environments: [prod, stg]
    bidirectional: false
  - from: app-server
    to: db-primary
...
function init(){
  ...
  $('btn-ae').addEventListener('click',openEnvDlg);
  $('env-close').addEventListener('click',closeEnvDlg);
  $('env-add-btn').addEventListener('click',addEnv);
